...
namespace neu
{
	struct Vector3
	{
		...
		//comparison
		//Vector3 == Vector3
		bool operator == (const Vector3& v) const { return (this->x == v.x && this->y == v.y && this->y == v.z); }
		bool operator != (const Vector3& v) const { return (this->x != v.x || this->y != v.y || this->y != v.z); }
	...
	inline float Vector3::Length() { return std::sqrt((x * x) + (y * y)); }
	...
	inline Vector3 Vector3::Normalized() { float length = Length(); return Vector3{ x / length, y / length, z / length }; }
