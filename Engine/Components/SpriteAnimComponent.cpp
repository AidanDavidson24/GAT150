...
{
    renderer.Draw(m_sequence->m_texture, GetSource(), m_owner->m_transform, registration);
}
...
{
    {
        ...
        if (value.HasMember("sequences") && value["sequences"].IsArray())
        {
            for (auto& sequenceValue : value["sequences"].GetArray())
            {
                ...
                sequence.m_texture = g_resources.Get<Texture>(texture_name, g_renderer);
...
void neu::SpriteAnimComponent::SetSequence(const std::string& name)
{
...
neu::Rect& neu::SpriteAnimComponent::GetSource()
{
    // calculate source rect 
    Vector2 cellSize = m_sequence->m_texture->GetSize() / Vector2{ m_sequence->num_columns, m_sequence->num_rows };
